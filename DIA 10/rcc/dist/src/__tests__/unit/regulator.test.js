...
describe('Unit: Regulator', () => {
    ...
    describe('regulate() - CALM state', () => {
        test('CALM returns PASSTHROUGH action', () => {
            const analysis = { state: 'CALM', score: 0.1, reason: 'LOW_INTENSITY' };
            const result = (0, regulator_1.regulate)(analysis, 'calm message');
        ...
        test('CALM includes meta with state info', () => {
            const analysis = { state: 'CALM', score: 0.15, reason: 'LOW_INTENSITY' };
            const result = (0, regulator_1.regulate)(analysis, 'test');
    ...
    describe('regulate() - NEUTRAL state', () => {
        test('NEUTRAL returns PASSTHROUGH action', () => {
            const analysis = { state: 'NEUTRAL', score: 0.5, reason: 'BASELINE' };
            const result = (0, regulator_1.regulate)(analysis, 'neutral message');
            expect(result.action).toBe('PASSTHROUGH');
        });
    ...
    describe('regulate() - TENSE state', () => {
        test('TENSE returns SOFTEN action', () => {
            const analysis = { state: 'TENSE', score: 0.8, reason: 'HIGH_INTENSITY' };
            const result = (0, regulator_1.regulate)(analysis, 'tense message');
        ...
        test('TENSE includes meta with state info', () => {
            const analysis = { state: 'TENSE', score: 0.85, reason: 'HIGH_INTENSITY' };
            const result = (0, regulator_1.regulate)(analysis, 'test');
            expect(result.meta).toBeDefined();
    ...
    describe('regulate() - fail-safe behavior', () => {
        ...
        test('invalid state returns PASSTHROUGH', () => {
            ...
            const result = (0, regulator_1.regulate)(badAnalysis, 'test message');
            expect(result.action).toBe('PASSTHROUGH');
            expect(result.message).toBe('test message');
        });
        test('missing state property returns PASSTHROUGH', () => {
            const badAnalysis = { score: 0.5, reason: 'TEST' };
            const result = (0, regulator_1.regulate)(badAnalysis, 'test');
            expect(result.action).toBe('PASSTHROUGH');
        });
        test('empty input string preserved', () => {
            const analysis = { state: 'NEUTRAL', score: 0.5, reason: 'BASELINE' };
            const result = (0, regulator_1.regulate)(analysis, '');
        ...
        test('never throws exception', () => {
            ...
            badInputs.forEach(([analysis, input]) => {
                expect(() => (0, regulator_1.regulate)(analysis, input)).not.toThrow();
            });
        });
    });
    // ─────────────────────────────────────────────────────────────
    ...
    describe('regulate() - determinism', () => {
        test('same analysis + input = same output', () => {
            ...
            const r1 = (0, regulator_1.regulate)(analysis, input);
            const r2 = (0, regulator_1.regulate)(analysis, input);
            expect(r1.action).toBe(r2.action);
            expect(r1.message).toBe(r2.message);
        });
        test('different states produce different actions', () => {
            const input = 'test';
            const calm = { state: 'CALM', score: 0.1, reason: 'LOW_INTENSITY' };
            const tense = { state: 'TENSE', score: 0.9, reason: 'HIGH_INTENSITY' };
            const calmResult = (0, regulator_1.regulate)(calm, input);
            const tenseResult = (0, regulator_1.regulate)(tense, input);
