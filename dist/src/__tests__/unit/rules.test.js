...
describe('Unit: Rules', () => {
    ...
    describe('applyRules() - output structure', () => {
        test('returns string', () => {
            const result = (0, rules_1.applyRules)('test', 'NEUTRAL');
    ...
    describe('applyRules() - CALM state', () => {
        test('CALM preserves text unchanged', () => {
            ...
            const result = (0, rules_1.applyRules)(input, 'CALM');
            expect(result).toBe(input);
        });
        test('CALM preserves empty string', () => {
            expect((0, rules_1.applyRules)('', 'CALM')).toBe('');
        });
        test('CALM preserves special characters', () => {
            const input = 'hello!!! @#$ world???';
            expect((0, rules_1.applyRules)(input, 'CALM')).toBe(input);
        });
    ...
    describe('applyRules() - NEUTRAL state', () => {
        test('NEUTRAL preserves text unchanged', () => {
            const input = 'hello world';
            const result = (0, rules_1.applyRules)(input, 'NEUTRAL');
            expect(result).toBe(input);
        });
        test('NEUTRAL preserves empty string', () => {
            expect((0, rules_1.applyRules)('', 'NEUTRAL')).toBe('');
        });
        test('NEUTRAL preserves special characters', () => {
            const input = 'test!!! ???';
            expect((0, rules_1.applyRules)(input, 'NEUTRAL')).toBe(input);
        });
    ...
    describe('applyRules() - TENSE state', () => {
        test('TENSE preserves text (current implementation)', () => {
            ...
            const result = (0, rules_1.applyRules)(input, 'TENSE');
            expect(result).toBe(input);
        });
        test('TENSE preserves empty string', () => {
            expect((0, rules_1.applyRules)('', 'TENSE')).toBe('');
        });
        test('TENSE preserves exclamation marks', () => {
            const input = 'THIS IS URGENT!!!';
            expect((0, rules_1.applyRules)(input, 'TENSE')).toBe(input);
        });
    ...
    describe('applyRules() - default case', () => {
        test('unknown state returns text unchanged', () => {
            ...
            const result = (0, rules_1.applyRules)(input, 'UNKNOWN');
            expect(result).toBe(input);
        });
        test('empty state returns text unchanged', () => {
            const input = 'test';
            const result = (0, rules_1.applyRules)(input, '');
            expect(result).toBe(input);
        });
    });
    // ─────────────────────────────────────────────────────────────
    ...
    describe('applyRules() - consistency', () => {
        test('all states preserve text in current implementation', () => {
            ...
            states.forEach(state => {
                expect((0, rules_1.applyRules)(input, state)).toBe(input);
            });
        });
        test('empty input returns empty for all states', () => {
            const states = ['CALM', 'NEUTRAL', 'TENSE'];
            states.forEach(state => {
                expect((0, rules_1.applyRules)('', state)).toBe('');
            });
        });
    });
    // ─────────────────────────────────────────────────────────────
    ...
    describe('applyRules() - determinism', () => {
        test('same input + state = same output', () => {
            ...
            const states = ['CALM', 'NEUTRAL', 'TENSE'];
            states.forEach(state => {
                const r1 = (0, rules_1.applyRules)(input, state);
                const r2 = (0, rules_1.applyRules)(input, state);
                expect(r1).toBe(r2);
            });
        });
        test('does not mutate input', () => {
            const input = 'original text';
            (0, rules_1.applyRules)(input, 'TENSE');
    ...
    describe('applyRules() - edge cases', () => {
        test('handles very long text', () => {
            ...
            const result = (0, rules_1.applyRules)(longText, 'NEUTRAL');
            expect(result).toBe(longText);
        });
        test('handles unicode characters', () => {
            const unicode = '你好世界 🌍 مرحبا';
            expect((0, rules_1.applyRules)(unicode, 'NEUTRAL')).toBe(unicode);
        });
        test('handles special characters', () => {
            const special = '<script>alert("xss")</script>';
            expect((0, rules_1.applyRules)(special, 'NEUTRAL')).toBe(special);
        });
        test('handles tabs and newlines', () => {
            const whitespace = 'hello\t\n\rworld';
            expect((0, rules_1.applyRules)(whitespace, 'NEUTRAL')).toBe(whitespace);
        });
    });
});
